...
func _physics_process(delta: float) -> void:
	...
	if player_in_range != null:
		facing_direction = -1.0 if player_in_range.global_position.x < global_position.x else 1.0

	$AnimatedSprite2D.flip_h = facing_direction > 0

	if player_in_range != null and _fire_cooldown <= 0.0 and $AnimatedSprite2D.animation != "attack":
		_attack()
...
func _on_frame_changed() -> void:
	if $AnimatedSprite2D.animation == "attack" and $AnimatedSprite2D.frame == LAUNCH_FRAME and not _bullet_fired_this_attack:
